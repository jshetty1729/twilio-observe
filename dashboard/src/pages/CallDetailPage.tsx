...
import { Box, Button, Heading, Text, Separator, Badge } from '@twilio-paste/core';
  ...
  return (
    <Box padding="space60" backgroundColor="colorBackgroundBody" minHeight="100vh">
      ...
      <Box marginTop="space40" marginBottom="space40">
        ...
        <Box display="flex" columnGap="space60" marginTop="space20" alignItems="center">
          <CsatIndicator score={detail.csat} />
          <Badge as="span" variant={
            ['negative', 'very_negative', 'frustrated', 'angry'].includes(detail.sentiment) ? 'error'
            : ['positive', 'very_positive', 'satisfied'].includes(detail.sentiment) ? 'success'
            : 'neutral'
          }>
            {detail.sentiment}
          </Badge>
      ...
      <Box display="flex" columnGap="space60">
        ...
        <Box width="350px">
          {detail.status !== 'barged' && (
            <>
              <CoachPanel callSid={callSid!} />
